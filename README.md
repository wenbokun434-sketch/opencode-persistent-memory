...
- 📦 **零数据库依赖** — 无需 Docker、无需外部数据库，直接落地 JSON 文件
- 🧠 **双模型沙箱防御** — 记忆提取使用独立的无工具权限 LLM 调用，物理隔绝 Prompt Injection
- 📐 **时间衰减算法** — 30 天半衰期，自动遗忘陈旧偏好
- 🔄 **双轨存储降级** — ruvector（Rust Native）→ vectorvault（纯 TypeScript 磁盘持久化），跨平台零编译依赖
- 🔧 **可配置 LLM 提供商** — 通过 `OPENCODE_MEMORY_PROVIDER` 环境变量选择 anthropic/openai/deepseek

> **注意**：记忆提取功能需要有效的 LLM API Key（配置在 `auth.json` 中）。记忆数据存储是本地化的。
...
在 `auth.json` 中配置 API Key（与 OpenCode 共享鉴权）：
...
| 降级存储 | vectorvault | 100% TypeScript，零原生依赖，JSON 文件持久化 |
...
- **记忆数据本地存储** — 记忆数据存储在 `~/.config/opencode/memory_store/`
